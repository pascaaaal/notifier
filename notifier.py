...
parser.add_argument(
    '-c', '--config', help='Provides the config file', required=True)
parser.add_argument(
    '-d', '--data', help='Provides the data file', required=True)
...
while True:
    ips = []
    proc = subprocess.Popen(['fping', '-d', '-i 1', '-r 2', '-a', '-g',
                             env_vars['subnet_addr']], stdout=subprocess.PIPE, stderr=open(os.devnull, 'w'))
